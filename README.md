...
Для проверки нейросети на тестовых данных используется скрипт validate_model.py. Выводятся значения IoU, а затем итоговый процент случаев, в котором IoU было ниже заданного порога.
